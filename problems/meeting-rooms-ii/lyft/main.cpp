...
int main(int argc, char** argv) {
    ...
    int cntJobs;
    fin >> cntJobs;
    vector<string> jobOrder;
    map<string /* startTime */, string /* endTime */> jobs;
    for (int i = 0; i < cntJobs; i++) {
        fin >> startTime >> duration;
        jobOrder.push_back(startTime);
        jobs[startTime] = s.calculateEndTime(startTime, duration);
    }
    map<string, int> jobWorker = s.getWorkerAssignment(jobs);
    cout << s.getWorkerCount() << endl;
    for (int i = 0; i < jobOrder.size(); i++) {
        cout << "J" << i << " " << " W" << jobWorker[jobOrder[i]] << endl;
    }
    return 0;
}